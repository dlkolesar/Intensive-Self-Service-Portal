using System;
using System.Collections.Generic;

namespace Intensive.Data.EBIDataMart
{
    public partial class DimSurveyAnswer
    {
        public int SurveyAnswerKey { get; set; }
        public string SurveyAnswerNk { get; set; }
        public string SurveyAnswer { get; set; }
        public int? SurveyAnswerWordCount { get; set; }
        public int? SurveyAnswerIsOther { get; set; }
        public DateTime EffectiveStartDate { get; set; }
        public DateTime EffectiveEndDate { get; set; }
        public DateTime RecordCreatedDatetime { get; set; }
        public string RecordCreatedBy { get; set; }
        public DateTime RecordUpdatedDatetime { get; set; }
        public string RecordUpdatedBy { get; set; }
        public string SourceSystemName { get; set; }
        public int CurrentRecord { get; set; }
    }
}
